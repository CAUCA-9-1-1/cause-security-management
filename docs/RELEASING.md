...
All four packable projects share a single, identical `<Version>` value:
...
| `Cause.SecurityManagement.Wolverine` | `Cause.SecurityManagement.Wolverine` |
...
Because all four packages share one version, apply the **highest required bump
across any package** to all of them.
...
Update all four `<Version>` elements to the same new value before releasing.

## Cross-Package Compatibility

A given version set (`10.2.0`, `10.3.0-preview1`, etc.) is fully compatible
across all four packages. Consuming applications should pin all
...
```xml
<PackageReleaseNotes>
10.3.0 — Adds group management API (Wolverine handlers, Core services, HTTP
controllers). All four packages must be upgraded together. Backwards compatible
with 10.2.x consumers that do not use the group management feature.
</PackageReleaseNotes>
```

## How to Release

1. Bump `<Version>` in all four packable `.csproj` files to the same new value.
...
- Pack only the four packable projects into `./artifacts/nupkg`.
